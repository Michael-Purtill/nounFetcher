...
const data = fs.readFileSync('./czechpreplinks.txt').toString();
...
for (var i = 0; i < myData.length	; i++) {
	...
	var wordType = "Preposition";
	
	var fu = myData[i].split("/");
	
	var word = decodeURIComponent(fu[fu.length - 1].split("#")[0]);
	
	if (czechHTML.find("[id*='" + "Preposition" + "']").val() != undefined) {
			wordType = "Preposition";
	}
	
	var meanings = [];

	var wordMeaningHTML = czechHTML.find("[id*='" + "Preposition" + "']").parent().next().next();
...
fs.writeFileSync("prepositions.json", JSON.stringify(ret), 'utf8');