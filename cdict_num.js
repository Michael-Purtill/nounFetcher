...
const data = fs.readFileSync('./czechnumlinks.txt').toString();
...
for (var i = 0; i < myData.length; i++) {
	...
	if (czechHTML.find("[id*='" + "Numeral" + "']").val() != undefined) {
			wordType = "Numeral";
	}
	...
	var wordMeaningHTML = czechHTML.find("[id*='" + "Numeral" + "']").parent().next().next();
...
fs.writeFileSync("numerals.json", JSON.stringify(ret), 'utf8');